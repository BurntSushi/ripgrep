...
/// really well in throughput oriented searches because it potentially
    ...
    fn extract_concat<'a, I: Iterator<Item = &'a Hir>>(&self, it: I) -> TSeq {
        ...
        for hir in it {
            ...
            if seq.is_inexact() {
                ...
                // it's impossible for the concatenation to ever match. So we
                // can just quit now.
                if seq.is_empty() {
                    return seq;
                }
...
mod tests {
    ...
    fn impossible() {
        ...
        // concatenation containing an expression that can never match) is
