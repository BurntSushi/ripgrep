...
macro_rules! eprintln_locked {
    ($($tt:tt)*) => {{
        {
            // This is a bit of an abstraction violation because we explicitly lock
            // STDOUT before printing to STDERR. This avoids interleaving lines within
            // ripgrep because `search_parallel` uses `termcolor`, which accesses the
            // same STDOUT lock when writing lines.
